...
def setup_vectorstore():
    ...
    """
    # Embeddings được tạo theo PROVIDER trong .env (openai/gemini/ollama/...)
    embeddings = get_embeddings()

    # Đọc toàn bộ knowledge base (data/knowledge_base.txt)
    text = load_knowledge_base()

    # Chia nhỏ để mỗi chunk vừa context window và tăng độ chính xác retrieval
    chunks = split_text(text, chunk_size=500, chunk_overlap=50)
    print(f"📚 Đã chia thành {len(chunks)} chunks")

    # Index chunks vào FAISS
    vectorstore = build_vectorstore(chunks, embeddings)
    return vectorstore


# ── 2. RAG Prompt Template ─────────────────────────────────────────────────
# System message nhận {context} từ retriever, human message nhận {question}.
...
def build_rag_chain(vectorstore):
    ...
    # Lấy 3 chunk gần nhất cho mỗi câu hỏi
    ...
    # LCEL: dict đầu vào chạy song song → prompt → llm → parse thành str
    chain = (
        {"context": retriever | format_docs, "question": RunnablePassthrough()}
        | RAG_PROMPT
        | llm
...
def main():
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS, 1):
        ...
        except Exception as e:                  # 1 câu lỗi không làm hỏng cả lượt chạy
            answer = f"[LỖI] {e}"
